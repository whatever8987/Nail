...
apiClient.interceptors.request.use((config: AxiosRequestConfig) => {
   // Explicitly check if headers is defined before accessing Authorization
   if (!config.headers) {
       config.headers = {};
   }
...
export const API = {
  auth: {
    ...
    register: async (data: RegisterData): Promise<AxiosResponse<User>> => {
      ...
      // This maps to ${API_BASE_URL}/api/register/
  ...
  user: {
    ...
    getProfile: async (): Promise<AxiosResponse<User>> => {
      // Use apiClient with the relative path defined in backend users.urls.py
      // This maps to ${API_BASE_URL}/api/user/me/
  ...
  chat: {
    sendMessage: async (messages: ChatMessage[], sessionId?: string): Promise<AxiosResponse<ChatResponse>> => {
