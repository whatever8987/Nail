...
export function Chatbot() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (
        botResponseData &&
        botResponseData.candidates &&
        botResponseData.candidates.length > 0 &&
        botResponseData.candidates[0].content &&
        botResponseData.candidates[0].content.parts &&
        botResponseData.candidates[0].content.parts.length > 0 &&
        botResponseData.candidates[0].content.parts[0].text
      ) {
        botMessageContent = botResponseData.candidates[0].content.parts[0].text;
      } else {
        console.warn("Received unexpected response format from AI:", botResponseData);
        botMessageContent += " (Unexpected format)";
      }
      ...
      if (axios.isAxiosError(error) && error.response) {
        ...
        errorMessage = backendError ? `Error: ${backendError}` : `API Error: ${error.response.status}`;
      } else {
          ...
          >
            <div className="flex items-center text-white flex-grow">
              ...
              <div>
                ...
                {!isMinimized && <p className="text-xs opacity-80">We typically reply in a few minutes</p>}
              </div>
            </div>
          ...
          {!isMinimized && (
            <>
              ...
              {messages.length <= initialMessages.length && !isTyping && (
                <div className="px-4 py-3 border-t border-gray-200 dark:border-gray-800 overflow-x-auto">
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">Common Questions:</p>
                  <div className="flex space-x-2 pb-2">
                    {commonQuestions.map((question, index) => (
                      <Button
                        ...
                        variant="outline"
                        size="sm"
                        className="text-xs rounded-full whitespace-nowrap flex-shrink-0 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700"
