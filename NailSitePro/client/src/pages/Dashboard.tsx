...
// ... other component imports like Button, Card, etc.
import {
  Loader2,
  Edit,
  Eye,
...
// You might define ReportOverviewData type here or import it from api.ts
// For now, let's import it from api.ts if you added it there
// import { ReportOverviewData } from "@/lib/api"; // Assuming you added it to api.ts
...
export default function Dashboard() {
        ...
        try {
            ...
            return response.data;
             ...
             if (error.response?.status === 401) {
                 ...
                 // or explicitly here if preferred, but interceptor is better.
                 // If using interceptor, just returning null is enough here.
  ...
  const { data: analyticsData, isLoading: isLoadingAnalytics, error: analyticsError } = useQuery<ReportOverviewData | null>({
    ...
    queryFn: async () => {
        try {
            ...
            // If it's a 401, the global interceptor should handle redirecting.
            // If it's a 403 (Forbidden), inform the user they lack permission.
            if (axios.isAxiosError(error) && error.response?.status === 403) {
                 // User is logged in, but doesn't have admin permission for this endpoint
                 // Show a toast or a message in the UI
            ...
            // Re-throw other errors or return null/error object
            throw error; // Let react-query handle other errors
        }
    },
    // Only enable this query if the user is logged in.
    // Also, only enable if they have the necessary permission (e.g., user?.role === 'admin')
    // The backend permission_classes should also enforce this.
    enabled: isLoggedIn && user?.role === 'admin', // Assuming only admins can see this report
    retry: 1, // Retry once on failure (excluding 401/403 handled above)
    staleTime: 5 * 60 * 1000,
    gcTime: 10 * 60 * 1000,
  });
  ...
  // If user is null after loading, redirect should have happened via interceptor/catch.
  // This return null is a safety fallback.
  if (!user) {
       return null;
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" onClick={() => navigate('/account')}>
               ...
               {/* Only show Edit Website if the user has a salon string associated */}
              {user.salon && (
        ...
        {/* Website Status (Simplified based on user.salon string) */}
        {/* This block remains as implemented in the previous fix */}
         <div className="mb-12">
               ...
               <>
                 <div className="p-6">
                   <div className="flex justify-between items-start">
                     <div>
                       <h3 className="text-xl font-bold text-foreground">Your Salon Website</h3>
                        {/* Displaying the salon identifier string from the user object */}
                        {user.salon && <p className="text-muted-foreground text-sm">Associated ID: {user.salon}</p>}
                     </div>
                     <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100">
                       <CheckCircle className="h-3 w-3 mr-1" /> Account Linked
                     </span>
                   </div>
                   <div className="mt-6">
            ...
            {isLoadingAnalytics ? (
              ...
              <>
                 ...
                 <div className="glass-card p-6 mb-8">
                    ...
                    {analyticsData.visits_by_day.length > 0 ? (
               ...
               // Message if user is admin but there's no data yet or initial load failed
                <div className="text-center p-8 text-muted-foreground">
                    No analytics data available yet or access denied.
                </div>
            )}
          </div>
        )}
